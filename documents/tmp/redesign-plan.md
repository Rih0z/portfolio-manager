...
- [ ] PWA対応（Service Worker、App Shell キャッシュ、「ホーム画面に追加」）
- [ ] 通知システム
- [ ] ハードコード色の修正（hover:text-white → hover:text-foreground 等 3箇所）
...
Phase 4:   差別化機能        4週間   ← 完了 ✅ (4-A: ゴール+月次レポート, 4-B: バンドル-65%, 4-C: Modern*→shadcn/ui, 4-D: レガシー全削除+ドキュメント刷新)
──────────────────────────────────────
