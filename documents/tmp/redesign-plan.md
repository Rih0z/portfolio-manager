...
**作成日**: 2026-03-03
**最終更新**: 2026-03-04（第4回レビュー反映 — モバイル戦略・バックエンド技術的負債・デプロイ品質管理・テスト戦略拡充）
...
- 完全無料、収益化コードゼロ
- React SPA (CRA) + AWS Lambda + DynamoDB
...
| **アーキテクチャ** | PortfolioContext.js 76KB。CRA。TypeScriptなし | 🟡 |
...
現状                          →  新設計
─────────────────────────────────────────────────
React 18 (JavaScript)         →  React 19 + TypeScript 5.x
react-scripts (CRA)           →  Vite 6.x
...
ヘルスチェックなし             →  GET /health エンドポイント追加
...
**A. 認証系ファイルの散乱（深刻度: 🟡）**
- `backend/src/function/auth/` に未使用の実験的ファイルが6つ残存
- googleDriveAuthDebug.js, googleDriveAuthWithLogging.js, googleDriveAuthDetailed.js,
  temporaryGoogleLogin.js, statelessGoogleLogin.js, basicGoogleLogin.js
- serverless.ymlで使用されているのは googleDriveAuth.js のみ
- → Phase 0-A追加タスクとして削除

**B. セッション→JWT移行の複雑さ（深刻度: 🟡）**
- 計画書は「1行」だが実際は以下の作業が必要:
  1. JWT生成・検証ミドルウェア作成
  2. Refresh Tokenのセキュアな保存（DynamoDB or httpOnly cookie）
  3. Google Driveトークン管理のセッションからの分離
  4. 既存セッションとの並行運用期間（再ログイン不要の移行パス）
  5. Cookie → Authorization Bearerヘッダーへのフロントエンド変更
  6. CORS設定の変更（credentials: true が不要になる可能性）
- → Phase 1を4〜5週間に拡大、またはPhase 0-DとしてJWT移行を分離
...
### Phase 0-A: ビルド基盤刷新（2週間）

**目標:** CRA→Vite移行。既存機能の完全動作維持
**完了条件:** Viteビルド成功、本番デプロイで全機能動作、既存Jestテストパス

- [ ] vite, @vitejs/plugin-react インストール
- [ ] vite.config.ts 作成（server.proxy, build.outDir='build'）
- [ ] index.html をルートに移動、%PUBLIC_URL%除去、`<script type="module">` 追加
- [ ] src/index.js → src/main.tsx リネーム
- [ ] react-scripts, @babel/core, @babel/preset-env, @babel/preset-react 削除
- [ ] craco.config.js 削除（2ファイル）
- [ ] setupProxy.js → vite.config.ts proxy設定に移行、削除
- [ ] package.json scripts 更新
- [ ] REACT_APP_* → VITE_* 全ファイル一括置換
- [ ] process.env.REACT_APP_* → import.meta.env.VITE_* 一括置換
- [ ] .env ファイル群 + CI/CDの環境変数更新
- [ ] require() → import文 変換（6ファイル）
- [ ] 不要ファイル削除（バックアップ、未使用ファイル）
- [ ] 市場データの免責表示UIを既存DataSourceBadgeコンポーネントに追加
- [ ] 未使用auth関連ファイル削除（6ファイル: debug/logging/detailed/temporary/stateless/basic）
- [ ] フロントエンド・バックエンドのテストカバレッジベースライン計測・記録
- [ ] **検証: 既存Jestテスト全パス + 本番デプロイ動作確認 + スモークテスト手動実行**
...
- [ ] typescript, @types/react, @types/react-dom インストール
- [ ] tsconfig.json（strict: false）, vite-env.d.ts 作成
- [ ] エントリーポイント → .tsx変換
- [ ] types/, services/, utils/, context/, hooks/ → .ts/.tsx変換
- [ ] コンポーネント: 新規・変更時に順次.tsx化
- [ ] vitest, @vitest/ui, jsdom インストール
- [ ] jest.fn()→vi.fn(), jest.mock()→vi.mock() 一括変換
- [ ] 旧jest config群削除
- [ ] **検証: Vitest全テストパス + 本番デプロイ動作確認**

### Phase 0-C: 状態管理刷新（3週間）

**目標:** PortfolioContext.js 76KB → Zustand 4ストア + TanStack Query
**完了条件:** 互換ラッパー経由で全機能動作

- [ ] zustand, @tanstack/react-query インストール
- [ ] usePortfolioStore, useAuthStore, useSimulationStore, useUIStore 作成
- [ ] TanStack Query カスタムフック作成（useExchangeRate, useStockPrice等）
- [ ] QueryClientProvider をApp.tsxに追加
- [ ] PortfolioContext互換ラッパー作成（内部でZustand+TanStack Query）
- [ ] コンポーネント段階移行
- [ ] Base64エンコード → Zustand persist移行
- [ ] データ精度モニタリング基盤（CloudWatch Metrics）
- [ ] **検証: 全テストパス + 全機能手動テスト + 本番デプロイ動作確認**
...
Phase 0-B: Jest→Vitest一括切替。カバレッジがベースライン以上であることを確認
...
**Week 1-2: JWT認証移行**
- [ ] JWT生成・検証ミドルウェア作成
- [ ] Refresh Tokenローテーション（DynamoDB保存）
- [ ] Google Driveトークン管理のセッションからの分離
- [ ] 既存セッションからの移行パス（再ログイン不要）
- [ ] フロントエンド: Cookie → Authorization Bearerヘッダー対応
- [ ] CORS設定の見直し
...
Phase 0-B: 型安全性          3週間
Phase 0-C: 状態管理          3週間
Phase 1:   収益化+認証刷新   4〜5週間 ← JWT移行の複雑さを反映（旧3週間）
Phase 2-A: UX+デザイン       5週間 ─┐
Phase 2-B: データ基盤        3週間 ─┘ 並行可
Phase 3:   AI強化+E2E基盤    3週間   ← Playwright導入を含む
Phase 4:   差別化機能        4週間
──────────────────────────────────────
直列合計: 約27〜28週間（6.8〜7ヶ月）
並行実行時: 約24〜25週間（6〜6.3ヶ月）
```
