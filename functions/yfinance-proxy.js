...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    // パラメータに応じてフォールバックデータを充実させる
    const params = event.queryStringParameters || {};
    if (params.exchange_rate) {
      const pair = params.exchange_rate;
      let rate = 150.0; // USDJPYのデフォルト値
      
      if (pair === 'JPYUSD') {
        rate = 1 / 150.0;
      }
      
      fallbackData.rate = rate;
