...
if [ -f "./scripts/setup-test-env.js" ]; then
...
if [ $GENERATE_CHART -eq 1 ] && [ $NO_COVERAGE -ne 1 ]; then
  ...
  # チャート生成スクリプトを実行
  chart_command="node ./scripts/generate-coverage-chart.js"
  if command -v cross-env >/dev/null 2>&1; then
  ...
  if eval "$full_cmd"; then
    ...
    if [ $DEBUG_MODE -eq 1 ]; then
      ...
      eval "$chart_env_vars node --trace-warnings ./scripts/generate-coverage-chart.js"
    fi
  fi
fi
