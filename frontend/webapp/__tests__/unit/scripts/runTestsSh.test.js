...
describe('run-tests.sh helper', () => {
  ...
  test('fails when no test type is given', () => {
    withTempSetup(tmp => {
      const result = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh')], { cwd: tmp, encoding: 'utf8' });
      expect(result.status).not.toBe(0);
  ...
  test('invalid coverage target exits with error', () => {
    withTempSetup(tmp => {
      const result = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '--target', 'foo', 'all'], { cwd: tmp, encoding: 'utf8' });
      expect(result.status).not.toBe(0);
  ...
  test('--clean removes previous reports', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '--clean', 'all'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('--chart triggers coverage chart generation', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '--chart', 'all'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('--html-coverage opens report', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '--html-coverage', 'all'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('falls back when cross-env is missing', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), 'all'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('runs specific pattern tests', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '-s', 'foo', 'specific'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('ignore coverage errors sets exit code to zero', () => {
    withTempSetup(tmp => {
      ...
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'scripts/run-tests.sh'), '-i', 'all'], { cwd: tmp, env, encoding: 'utf8' });
