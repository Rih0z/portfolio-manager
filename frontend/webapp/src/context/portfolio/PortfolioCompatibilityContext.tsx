...
import type { Notification, SimulationResult } from '../../types/portfolio.types';

export const PortfolioContext = createContext<any>(undefined);

interface PortfolioCompatibilityProviderProps {
  children: ReactNode;
}
 ...
 */
export const PortfolioCompatibilityProvider: React.FC<PortfolioCompatibilityProviderProps> = ({ children }) => {
  const dataContext = useContext(PortfolioDataContext) as any;
  const actionsContext = useContext(PortfolioActionsContext) as any;
  ...
  const [includeCurrentHoldings, setIncludeCurrentHoldings] = useState<boolean>(true);

  const cloudProvider = new GoogleDriveProvider();

  // 市場データの更新
  const refreshMarketData = useCallback(async (): Promise<any> => {
    ...
    try {
      const tickers = dataContext.currentAssets.map((asset: any) => asset.ticker);
      const result = await fetchMultipleStocks(tickers, true);

      if (result.success) {
        const updatedAssets = dataContext.currentAssets.map((asset: any) => {
  ...
  const updateExchangeRate = useCallback(async (forceUpdate: boolean = false): Promise<void> => {
    try {
      const result: any = await fetchExchangeRate('USD', 'JPY', forceUpdate);
  ...
  // シミュレーション実行
  const runSimulation = useCallback((): any => {
  ...
  // 購入実行
  const executePurchase = useCallback((): void => {
...
interface PortfolioCompatibilityWrapperProps {
  children: ReactNode;
}
 ...
 */
export const PortfolioCompatibilityWrapper: React.FC<PortfolioCompatibilityWrapperProps> = ({ children }) => {
  return (
