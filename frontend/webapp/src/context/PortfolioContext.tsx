 ...
 * ファイルパス: src/context/PortfolioContext.tsx
 *
 ...
 *
 * NOTE: このファイルは~1866行あり、UI/UXリデザイン時にリライト予定。
 * 型は `any` を自由に使用。
 */
...
// コンテキストの作成
export const PortfolioContext = createContext<any>(undefined);
...
export const PortfolioProvider: React.FC<PortfolioProviderProps> = ({ children }) => {
  ...
  // 通知を追加する関数（タイムアウト付き）
  const addNotification = useCallback((message: string, type: string = 'info'): string => {
  ...
  // AI分析プロンプトテンプレート更新関数
  const updateAiPromptTemplate = useCallback((template: any): void => {
  ...
  const saveToLocalStorage = useCallback((): boolean => {
    ...
    try {
      ...
      console.log('ローカルストレージにデータを保存しました', {
        ...portfolioData,
        currentAssetsLength: portfolioData.currentAssets?.length,
  ...
  const loadFromLocalStorage = useCallback((): any | null => {
    try {
      ...
      // 為替レートオブジェクトの整合性チェック
      if (decryptedData.exchangeRate) {
        if (typeof decryptedData.exchangeRate !== 'object' || decryptedData.exchangeRate === null) {
          console.warn('為替レートデータが不正な形式です。デフォルト値に置き換えます。');
          decryptedData.exchangeRate = {
            rate: 150.0,
            lastUpdated: new Date().toISOString(),
            isDefault: true,
            source: 'fallback'
          };
        } else if (!decryptedData.exchangeRate.rate || typeof decryptedData.exchangeRate.rate !== 'number') {
          console.warn('為替レートのrateプロパティが無効です。デフォルト値を設定します。');
          decryptedData.exchangeRate.rate = 150.0;
          decryptedData.exchangeRate.isDefault = true;
          decryptedData.exchangeRate.source = 'fallback';
        }
      } else {
        console.warn('為替レートデータが存在しません。デフォルト値を設定します。');
        decryptedData.exchangeRate = {
          rate: 150.0,
          lastUpdated: new Date().toISOString(),
          isDefault: true,
  ...
  // 通貨切替
  const toggleCurrency = useCallback((): void => {
    setBaseCurrency(prev => {
      const newCurrency = prev === 'JPY' ? 'USD' : 'JPY';
      console.log(`通貨を切り替えます: ${prev} -> ${newCurrency}`);
      // 通貨切替後、自動保存
  ...
  const validateAssetTypes = useCallback((assets: any[]): { updatedAssets: any[]; changes: any } => {
  ...
  // 市場データの更新（レート制限付き）
  const refreshMarketPricesInternal = useCallback(async (): Promise<any> => {
    setIsLoading(true);
    try {
      ...
      const fundTypeChangeDetails: any[] = []; // 銘柄タイプの変更詳細
      ...
      let fallbackCount = 0;
      const fallbackDetails: any[] = [];

      // Yahoo Financeの利用統計
      let yahooFinanceTriedCount = 0;
      let yahooFinanceSuccessCount = 0;
      const yahooFinanceDetails: any[] = [];

      // 全銘柄のティッカーを取得
      const tickers = currentAssets.map((asset: any) => asset.ticker);
      ...
      // データソースの統計を計算
      const sourceCounts = validatedAssets.reduce((acc: any, asset: any) => {
      ...
      if (errorCount > 0) {
        ...
        // 最大3件までエラー詳細を表示
        errorDetails.slice(0, 3).forEach((error: any) => {
          addNotification(
      ...
      if (fallbackCount > 0) {
        ...
        // 最大3件までフォールバック詳細を表示
        fallbackDetails.slice(0, 3).forEach((detail: any) => {
          addNotification(
      ...
      if (fundTypeChangesCount > 0) {
        ...
        // 詳細な変更内容も表示（最大3件まで）
        fundTypeChangeDetails.slice(0, 3).forEach((detail: any) => {
          addNotification(
      ...
      if (feeChangesCount > 0) {
        ...
        // 詳細な変更内容も表示（最大3件まで）
        feeChangeDetails.slice(0, 3).forEach((detail: any) => {
  ...
  const refreshMarketPrices = useCallback(async (): Promise<any> => {
    return debouncedRefreshMarketData(refreshMarketPricesInternal);
  }, [refreshMarketPricesInternal]);

  // 銘柄追加（銘柄タイプ、手数料、配当情報の設定を含む）
  const addTicker = useCallback(async (ticker: string): Promise<any> => {
    try {
      // 既に存在するか確認
      ...
      const tickerResult: any = await requestManager.request(
        'alphaVantage',
      ...
      // ファンド情報を取得（手数料率など）
      const fundInfoResult: any = await requestManager.request(
        'default',
      ...
      // 配当情報を取得
      const dividendResult: any = await requestManager.request(
        'default',
  ...
  // 目標配分更新
  const updateTargetAllocation = useCallback((id: string, percentage: number | string): void => {
    setTargetPortfolio((prev: any[]) => {
  ...
  // 保有数量更新（小数点以下4桁まで対応）
  const updateHoldings = useCallback((id: string, holdings: number | string): void => {
    setCurrentAssets((prev: any[]) => {
      const updated = prev.map((item: any) =>
  ...
  const updateAnnualFee = useCallback((id: string, fee: number | string): void => {
    setCurrentAssets((prev: any[]) => {
      const updated = prev.map((item: any) => {
        if (item.id === id) {
          ...
          if (item.isStock || item.fundType === FUND_TYPES.STOCK) {
            ...
            return {
              ...item,
              annualFee: parseFloat(parseFloat(String(fee)).toFixed(2)) || 0,
              feeSource: 'ユーザー設定',
              feeIsEstimated: false
            };
          }
  ...
  // 配当情報の更新
  const updateDividendInfo = useCallback((id: string, dividendYield: number | string, hasDividend: boolean = true, frequency: string = 'quarterly'): void => {
    setCurrentAssets((prev: any[]) => {
      const updated = prev.map((item: any) => {
        if (item.id === id) {
          return {
  ...
  const removeTicker = useCallback((id: string): void => {
    setTargetPortfolio((prev: any[]) => prev.filter((item: any) => item.id !== id));
    setCurrentAssets((prev: any[]) => {
  ...
  // 通貨変換関数（新規追加）
  const convertCurrency = useCallback((amount: number, fromCurrency: string, toCurrency: string, exchangeRateObj: any): number => {
    ...
    if (fromCurrency === 'JPY' && toCurrency === 'USD') {
      return amount / exchangeRateObj.rate;
    }
  ...
  // 購入株数の計算（新規追加）
  const calculatePurchaseShares = useCallback((purchaseAmount: number, price: number): number => {
  ...
  // シミュレーション計算（マルチ通貨対応版）
  const calculateSimulation = useCallback((): any[] => {
    // 総資産額計算
    ...
    const results = targetPortfolio.map((target: any) => {
      // 現在の資産から対応する銘柄を検索
      const asset = currentAssets.find((asset: any) => asset.ticker === target.ticker);
  ...
  // 購入処理（小数点以下4桁まで対応）
  const executePurchase = useCallback((tickerId: string, units: number | string): void => {
    setCurrentAssets((prev: any[]) => {
      const updated = prev.map((asset: any) =>
        asset.id === tickerId
          ? { ...asset, holdings: parseFloat((asset.holdings + parseFloat(String(units))).toFixed(4)) }
          : asset
      );
      // 変更後に自動保存
      setTimeout(() => saveToLocalStorage(), 100);
  ...
  // 一括購入処理
  const executeBatchPurchase = useCallback((simulationResult: any[]): void => {
    simulationResult.forEach((result: any) => {
  ...
  // データのエクスポート
  const exportData = useCallback((): any => {
    return {
      baseCurrency,
      exchangeRate,
      lastUpdated,
  ...
  // 為替レートの更新（キャッシュ機能付き）
  const updateExchangeRate = useCallback(async (forceUpdate: boolean = false): Promise<void> => {
    // デバウンスチェック
    if (!shouldUpdateExchangeRate(forceUpdate)) {
      return;
    }
    ...
    try {
      ...
      if (cached && !forceUpdate) {
        try {
          ...
          // キャッシュデータの妥当性チェック
          if (cachedData.data && typeof cachedData.data.rate === 'number' && cachedData.data.rate > 0) {
            ...
            console.warn('無効な為替レートキャッシュを検出しました。クリアします。');
      ...
      if (currentBaseCurrency === 'JPY') {
        const result: any = await requestManager.request(
          'exchangeRate',
        ...
        if (result.success) {
          ...
          if (result.isDefault || result.isStale) {
            const warningMessage = result.isDefault
              ? 'デフォルトの為替レート（150円/ドル）を使用しています。'
              : '為替レートが古い可能性があります。';
            addNotification(warningMessage, 'warning');
          }
        ...
        if (result.success) {
          const rateData = {
            rate: result.rate,  // USD/JPYレート（例：150）をそのまま使用
            source: result.source,
            lastUpdated: result.lastUpdated
          };
          setExchangeRate(rateData);
  ...
  // 為替レートを手動でリセット（データ破損時の修復用）
  const resetExchangeRate = useCallback((): void => {
    console.log('為替レートを手動でリセットします');
    ...
    addNotification('為替レートをデフォルト値（150円/ドル）にリセットしました', 'info');

    // リセット後にAPIから最新レートを取得（強制更新）
    setTimeout(() => {
      updateExchangeRate(true); // forceUpdate = true
    }, 1000);

    // ローカルストレージに保存
    setTimeout(() => saveToLocalStorage(), 500);
  }, [addNotification, saveToLocalStorage, updateExchangeRate]);

  // Google認証状態の変更を処理
  const handleAuthStateChange = useCallback((isAuthenticated: boolean, user: any): void => {
    // 認証状態変更時の処理
  ...
  const saveToGoogleDrive = useCallback(async (userData: any = null): Promise<any> => {
  ...
  const loadFromGoogleDrive = useCallback(async (userData: any = null): Promise<any> => {
    // userDataが渡されない場合は、currentUserを使用
    const user = userData || currentUser;
  ...
  const initializeData = useCallback((): void => {
    try {
      ...
      if (localData) {
        ...
        if (localData.exchangeRate) {
          console.log('為替レートを設定:', localData.exchangeRate);
          // rateプロパティが欠けている場合はデフォルト値を設定
          if (!localData.exchangeRate.rate) {
            console.warn('為替レートのrateプロパティが欠けています。デフォルト値を設定します。');
            localData.exchangeRate.rate = 150.0;
          }
          setExchangeRate(localData.exchangeRate);
        }
        ...
        if (Array.isArray(localData.currentAssets)) {
          ...
          if (changes.fundType > 0) {
            ...
            // 詳細な変更内容も表示（最大3件まで）
            changes.fundTypeDetails.slice(0, 3).forEach((detail: any) => {
              addNotification(
                `「${detail.name || detail.ticker}」のタイプが「${detail.oldType}」から「${detail.newType}」に変更されました`,
                'info'
              );
  ...
  }, [loadFromLocalStorage, addNotification, validateAssetTypes, saveToLocalStorage, updateExchangeRate]);
  ...
  // 総資産額の計算
  const totalAssets = currentAssets.reduce((sum: number, asset: any) => {
    let assetValue = asset.price * asset.holdings;
    ...
    if (isNaN(assetValue)) {
      console.error('資産額がNaNです:', {
        ...
        calculation: `${asset.price} * ${asset.holdings} * ${asset.currency !== baseCurrency ? exchangeRate.rate : 1}`
      });
      return sum;
    }
  ...
  // 年間手数料の計算
  const annualFees = currentAssets.reduce((sum: number, asset: any) => {
    // 個別株は手数料がかからない
  ...
  // 年間配当金の計算（新規）
  const annualDividends = currentAssets.reduce((sum: number, asset: any) => {
    // 配当がない場合はスキップ
  ...
  // 追加予算設定（通貨指定に対応）
  const setAdditionalBudgetWithCurrency = useCallback((amount: number | string, currency?: string): void => {
    setAdditionalBudget({
  ...
  const contextValue: any = {
    ...
    updateExchangeRate, // 為替レート更新関数を追加
    resetExchangeRate, // 為替レートリセット関数を追加
    // データ保存・同期関連
    saveToLocalStorage,
    ...
    refreshMarketData: refreshMarketPrices,
    setBaseCurrency,
    setAiPromptTemplate,
    // simulation aliases
    ...
    runSimulation: calculateSimulation,
    // CRUD aliases
    updateHolding: updateHoldings,
    removeAsset: removeTicker,
  };
