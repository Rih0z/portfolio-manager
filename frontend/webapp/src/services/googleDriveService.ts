...
interface FetchDriveFilesResponse {
  success: boolean;
  files: DriveFile[];
  count?: number;
  error?: string;
...
interface LoadFromDriveResponse {
  success: boolean;
  data?: any;
  file?: DriveFile;
  message?: string;
...
export const fetchDriveFiles = async (): Promise<FetchDriveFilesResponse> => {
  try {
...
export const saveToDrive = async (portfolioData: any): Promise<SaveToDriveResponse> => {
  try {
...
export const loadFromDrive = async (fileId: string): Promise<LoadFromDriveResponse> => {
  try {
