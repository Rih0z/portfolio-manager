/**
 * プロジェクト: https://portfolio-wise.com/
 * ファイルパス: src/services/marketDataService.ts
 *
 * 作成者: Koki Riho （https://github.com/Rih0z）
 * 作成日: 2025-05-11 17:00:00
 *
 * 更新履歴:
 * - 2025-05-11 17:00:00 Koki Riho 初回作成
 * - 2025-05-12 10:30:00 外部APIサーバー利用に修正
 * - 2025-05-19 12:15:00 System Admin AWS環境対応に修正
 *
 * 説明:
 * 市場データ取得のための集約API呼び出しを提供するサービス。
 * 株価データ、為替レート、ファンド情報、配当データなどを
 * 一元的に取得する機能を提供する。
 */

import { getApiEndpoint } from '../utils/envUtils';
import { fetchWithRetry, formatErrorResponse, generateFallbackData, TIMEOUT } from '../utils/apiUtils';
import { getErrorMessage, getErrorStatus } from '../utils/errorUtils';
import logger from '../utils/logger';

interface ExchangeRateResponse {
  success: boolean;
  rate: number;
  source: string;
  lastUpdated: string;
  isDefault?: boolean;
  isStale?: boolean;
  warnings?: any[];
  error?: boolean;
  message?: string;
}

interface StockDataResponse {
  success: boolean;
  data?: Record<string, any>;
  source?: string;
  message?: string;
  error?: boolean;
  errors?: any[];
}

interface MultipleStocksResponse {
  success: boolean;
  data: Record<string, any>;
  errors: any[];
  sources: Record<string, number>;
  sourcesSummary?: string;
  message?: string;
  degraded?: boolean;
}

interface ApiStatusResponse {
  success: boolean;
  error?: any;
  [key: string]: any;
}

// 通貨換算レート取得
export const fetchExchangeRate = async (fromCurrency: string = 'USD', toCurrency: string = 'JPY', refresh: boolean = false): Promise<ExchangeRateResponse> => {
  try {
    const endpoint = await getApiEndpoint('api/market-data');
    const response = await fetchWithRetry(
      endpoint,
      {
        type: 'exchange-rate',
        base: fromCurrency,
        target: toCurrency,
        refresh: refresh ? 'true' : 'false'
      },
      TIMEOUT.EXCHANGE_RATE
    );

    // バックエンドから返されたデータの形式をチェック
    if (response.success && response.data) {
      const pairKey = `${fromCurrency}-${toCurrency}`;
      const exchangeData = response.data[pairKey];

      if (exchangeData) {
        // 警告があるかチェック
        if (exchangeData.isDefault || exchangeData.isStale || exchangeData.error) {
          logger.warn(`為替レート警告 (${pairKey}):`, {
            isDefault: exchangeData.isDefault,
            isStale: exchangeData.isStale,
            error: exchangeData.error
          });
        }

        // フロントエンドが期待する形式に変換
        return {
          success: true,
          rate: exchangeData.rate,
          source: exchangeData.isDefault ? 'Default' : 'API',
          lastUpdated: exchangeData.lastUpdated || new Date().toISOString(),
          isDefault: exchangeData.isDefault,
          isStale: exchangeData.isStale,
          warnings: response.warnings
        };
      }
    }

    // データが期待通りでない場合は元の形式をそのまま返す
    return response;
  } catch (error: unknown) {
    logger.error(`Error fetching exchange rate ${fromCurrency}/${toCurrency}:`, error);

    // フォールバック値を返す
    const errorResponse = formatErrorResponse(error as any);
    return {
      ...errorResponse,
      // デフォルト値も含める
      rate: fromCurrency === 'USD' && toCurrency === 'JPY' ? 150.0 :
            fromCurrency === 'JPY' && toCurrency === 'USD' ? 1/150.0 : 1.0,
      source: 'Fallback',
      lastUpdated: new Date().toISOString()
    };
  }
};

// 単一銘柄データ取得
export const fetchStockData = async (ticker: string, refresh: boolean = false): Promise<StockDataResponse> => {
  // 銘柄タイプに基づくタイムアウト設定
  let type: string = 'us-stock';
  let timeout: number = TIMEOUT.US_STOCK;

  // 日本株判定（数字4桁 + オプションの.T）
  if (/^\d{4}(\.T)?$/.test(ticker)) {
    type = 'jp-stock';
    timeout = TIMEOUT.JP_STOCK;
  }
  // 投資信託判定（複数のパターンに対応）
  // パターン1: 8桁の数字（例：03311187）
  // パターン2: 7桁の数字+アルファベット1文字（例：0331418A）
  // パターン3: アルファベット+数字の組み合わせ（例：9C31116A）
  else if (/^(\d{8}|\d{7}[A-Z]|[A-Z0-9]{8})$/i.test(ticker)) {
    type = 'mutual-fund';
    timeout = TIMEOUT.MUTUAL_FUND;
  }

  try {
    logger.debug(`Attempting to fetch data for ${ticker} from Market Data API`);
    const endpoint = await getApiEndpoint('api/market-data');
    const response = await fetchWithRetry(
      endpoint,
      {
        type,
        symbols: ticker,
        refresh: refresh ? 'true' : 'false'
      },
      timeout
    );

    return response;
  } catch (error: unknown) {
    logger.error(`Error fetching ${ticker} from Market Data API:`, {
      message: getErrorMessage(error),
      status: getErrorStatus(error),
      data: (error as any)?.response?.data,
      code: (error as any)?.code
    });

    // フォールバック処理
    const tickerErrorResponse = formatErrorResponse(error as any, ticker);
    return {
      ...tickerErrorResponse,
      // フォールバックデータも含める
      data: {
        [ticker]: generateFallbackData(ticker)
      },
      source: 'Fallback'
    };
  }
};

// 複数銘柄データ一括取得
export const fetchMultipleStocks = async (tickers: string[], refresh: boolean = false): Promise<MultipleStocksResponse> => {
  if (!tickers || tickers.length === 0) {
    return { success: true, data: {}, errors: [], sources: {} };
  }

  // 銘柄をタイプ別に分類
  const jpStocks: string[] = tickers.filter(ticker => /^\d{4}(\.T)?$/.test(ticker));
  const mutualFunds: string[] = tickers.filter(ticker => /^(\d{8}|\d{7}[A-Z]|[A-Z0-9]{8})$/i.test(ticker));
  const usStocks: string[] = tickers.filter(ticker =>
    !(/^\d{4}(\.T)?$/.test(ticker)) && !(/^(\d{8}|\d{7}[A-Z]|[A-Z0-9]{8})$/i.test(ticker))
  );

  const results: MultipleStocksResponse = {
    success: true,
    data: {},
    errors: [],
    sources: {}
  };

  // 並列にAPI呼び出し
  const requests: Promise<void>[] = [];

  if (usStocks.length > 0) {
    requests.push(
      fetchStockBatch('us-stock', usStocks, refresh, TIMEOUT.US_STOCK)
      .then(data => {
        Object.assign(results.data, data.data || {});
        if (data.errors) results.errors = [...results.errors, ...data.errors];
        if (data.source) {
          results.sources[data.source] = (results.sources[data.source] || 0) + Object.keys(data.data || {}).length;
        }
      })
      .catch(error => {
        logger.error('Error fetching US stocks:', error);
        // フォールバック処理
        usStocks.forEach(ticker => {
          results.data[ticker] = generateFallbackData(ticker);
          results.sources['Fallback'] = (results.sources['Fallback'] || 0) + 1;
        });
      })
    );
  }

  if (jpStocks.length > 0) {
    requests.push(
      fetchStockBatch('jp-stock', jpStocks, refresh, TIMEOUT.JP_STOCK)
      .then(data => {
        Object.assign(results.data, data.data || {});
        if (data.errors) results.errors = [...results.errors, ...data.errors];
        if (data.source) {
          results.sources[data.source] = (results.sources[data.source] || 0) + Object.keys(data.data || {}).length;
        }
      })
      .catch(error => {
        logger.error('Error fetching JP stocks:', error);
        // フォールバック処理
        jpStocks.forEach(ticker => {
          results.data[ticker] = generateFallbackData(ticker);
          results.sources['Fallback'] = (results.sources['Fallback'] || 0) + 1;
        });
      })
    );
  }

  if (mutualFunds.length > 0) {
    requests.push(
      fetchStockBatch('mutual-fund', mutualFunds, refresh, TIMEOUT.MUTUAL_FUND)
      .then(data => {
        Object.assign(results.data, data.data || {});
        if (data.errors) results.errors = [...results.errors, ...data.errors];
        if (data.source) {
          results.sources[data.source] = (results.sources[data.source] || 0) + Object.keys(data.data || {}).length;
        }
      })
      .catch(error => {
        logger.error('Error fetching mutual funds:', error);
        // フォールバック処理
        mutualFunds.forEach(ticker => {
          results.data[ticker] = generateFallbackData(ticker);
          results.sources['Fallback'] = (results.sources['Fallback'] || 0) + 1;
        });
      })
    );
  }

  // すべてのリクエストが完了するのを待つ
  await Promise.all(requests);

  // 結果をソースごとに集計
  results.sourcesSummary = Object.entries(results.sources)
    .map(([source, count]) => `${source}: ${count}件`)
    .join(', ');

  return results;
};

// バッチでのデータ取得（内部関数）
const fetchStockBatch = async (type: string, symbols: string[], refresh: boolean, timeout: number): Promise<any> => {
  try {
    const endpoint = await getApiEndpoint('api/market-data');
    return await fetchWithRetry(
      endpoint,
      {
        type,
        symbols: symbols.join(','),
        refresh: refresh ? 'true' : 'false'
      },
      timeout
    );
  } catch (error) {
    logger.error(`Error in fetchStockBatch (${type}):`, error);
    throw error;
  }
};

// ステータス取得（管理者用）
export const fetchApiStatus = async (): Promise<ApiStatusResponse> => {
  try {
    const endpoint = await getApiEndpoint('admin/status');
    const response = await fetchWithRetry(
      endpoint,
      {
        // APIキーはAWSから動的に管理されるため、ここでは指定しない
      }
    );
    return response;
  } catch (error) {
    logger.error('Error fetching API status:', error);
    return { success: false, error: formatErrorResponse(error as any) };
  }
};

export default {
  fetchExchangeRate,
  fetchStockData,
  fetchMultipleStocks,
  fetchApiStatus
};
