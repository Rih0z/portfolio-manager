...
interface TestTickers {
  mutualFunds: string[];
  jpStocks: string[];
}

// テスト対象の銘柄
const TEST_TICKERS: TestTickers = {
  mutualFunds: [
...
// 価格取得テスト関数
export const testJapaneseTickers = async (): Promise<void> => {
  ...
  // 一括取得テスト
  console.log('\n\n【一括取得テスト】');
  const allTickers: string[] = [...TEST_TICKERS.mutualFunds, ...TEST_TICKERS.jpStocks];
...
if (typeof window !== 'undefined') {
  (window as any).testJapaneseTickers = testJapaneseTickers;
