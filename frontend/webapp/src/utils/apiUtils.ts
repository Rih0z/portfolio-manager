...
class CircuitBreaker {
  ...
  constructor(name: string, threshold: number = RETRY.CIRCUIT_BREAKER_THRESHOLD, timeout: number = RETRY.CIRCUIT_BREAKER_TIMEOUT) {
    this.name = name;
    this.state = 'CLOSED';
    this.failureCount = 0;
...
// サーキットブレーカーの取得または作成
const getCircuitBreaker = (name: string): CircuitBreaker => {
...
export const refreshAccessToken = async (): Promise<string | null> => {
  if (isRefreshing) {
    ...
    return new Promise<string | null>((resolve) => {
      refreshSubscribers.push((token: string | null) => resolve(token));
    });
  }
...
// マーケットデータAPI用のクライアント
export const marketDataClient: AxiosInstance = createApiClient(false);

// 認証が必要なAPI用のクライアント
export const authApiClient: AxiosInstance = createApiClient(true);
...
export const fetchWithRetry = async (
  ...
  delayFn: (ms: number) => Promise<void> = wait
): Promise<any> => {
  let retries = 0;
...
// エラーレスポンスをフォーマットする関数
export const formatErrorResponse = (error: any, ticker?: string): ErrorResponse => {
