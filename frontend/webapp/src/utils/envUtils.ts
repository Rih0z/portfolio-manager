...
const getApiConfig = async (): Promise<ApiConfig> => {
  if (!apiConfigCache) {
    try {
      ...
      logger.warn('API設定の取得に失敗しました（フォールバック設定を使用）:', getErrorMessage(error));
  ...
  return apiConfigCache || ({} as ApiConfig);
};

// 環境に応じたベースURLの取得（非同期）
export const getBaseApiUrl = async (): Promise<string> => {
...
export const getApiEndpoint = async (path: string): Promise<string> => {
  const config = await getApiConfig();
...
export const getGoogleClientId = async (): Promise<string> => {
  try {
    ...
    logger.warn('Google Client ID の取得に失敗しました（フォールバックを使用）:', getErrorMessage(error));
    return 'dummy-client-id-for-development';
  }
};
...
// リダイレクトURIを生成
export const getRedirectUri = (path: string = '/auth/callback'): string => {
...
export const getDefaultExchangeRate = (): number => {
  const defaultRate = parseFloat((import.meta as any).env.VITE_DEFAULT_EXCHANGE_RATE);
