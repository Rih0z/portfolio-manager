...
// ファンドタイプごとの平均年間手数料率（％）
export const FUND_TYPE_FEES: Record<string, number> = {
...
// 明示的に米国ETFリストを定義
export const US_ETF_LIST: string[] = [
...
export const TICKER_SPECIFIC_FEES: Record<string, number> = {
  // 日本のETF
  '1306.T': 0.11, // TOPIX
  '1320.T': 0.16, // 日経225
  '1330.T': 0.09, // 日経225ミニ
...
export const TICKER_SPECIFIC_DIVIDENDS: Record<string, number> = {
  // 米国の主要ETF
...
export const EMAXIS_FUNDS: string[] = [
...
export const ETF_PREFIXES: string[] = [
...
// 日本の証券コード接頭辞マッピング
export const JP_CODE_PREFIX_MAP: Record<string, string> = {
  '1': 'ETF', // 通常のETF
...
const containsFundIndicators = (name: string): boolean => {
...
// ETFかどうかを判定
const isETF = (name: string, ticker: string): boolean => {
...
// REITかどうかを判定
const isREIT = (name: string): boolean => {
...
// 債券ファンドかどうかを判定
const isBond = (name: string, ticker: string): boolean => {
...
// 暗号資産関連かどうかを判定
const isCrypto = (name: string, ticker: string): boolean => {
...
const isActiveOrIndex = (name: string, isJapanese: boolean): string => {
  if (isActive(name)) {
    if (isGlobal(name)) {
      return FUND_TYPES.ACTIVE_GLOBAL;
    } else {
...
export const estimateAnnualFee = (ticker: string, name: string = ''): FeeEstimate => {
  if (!ticker) return {
...
export const extractFundInfo = (ticker: string, name: string = ''): FundInfo => {
  const info: Partial<FundInfo> = {};
  ticker = ticker.toUpperCase();
  name = (name || '').toLowerCase();
...
// 推定配当利回りを計算する
export const estimateDividendYield = (ticker: string, name: string = ''): DividendEstimate => {
  if (!ticker) return {
    yield: 0,
    isEstimated: true,
    hasDividend: false,
