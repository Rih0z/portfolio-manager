 ...
 * ファイルパス: src/components/layout/DataStatusBar.jsx 
 * 
 ...
 * 最終更新日時、為替レート情報、データ更新が必要かどうかの警告を表示する。
 * データ更新ボタンも含み、更新中の状態も視覚的に表示する。
 */
          ...
          {exchangeRate && (
            <>
              ...
              <span className={`font-medium ${
                exchangeRate.source === 'Default' ? 'text-yellow-600' : 
                exchangeRate.source === 'Fallback' ? 'text-orange-600' : 
                'text-gray-900'
              }`}>
              ...
              {exchangeRate.source && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                  exchangeRate.source === 'Default' ? 'bg-yellow-100 text-yellow-800' :
                  exchangeRate.source === 'Fallback' ? 'bg-orange-100 text-orange-800' :
