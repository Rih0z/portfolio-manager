...
const BudgetInput = () => {
  ...
  return (
    <div className="bg-card rounded-lg shadow border border-border p-4 mb-6">
      <h2 className="text-lg font-semibold mb-4 text-foreground">追加投資シミュレーション</h2>
      ...
      <div className="mb-4">
        ...
        <div className="flex space-x-4">
          <label className="inline-flex items-center">
            ...
            />
            <span className="ml-2 text-sm text-muted-foreground">円 (¥)</span>
          </label>
          <label className="inline-flex items-center">
            <input
              type="radio"
              ...
              className="form-radio h-4 w-4 text-blue-600"
            />
            <span className="ml-2 text-sm text-muted-foreground">ドル ($)</span>
          </label>
        </div>
      </div>
      
      <div className="mb-4">
        <label htmlFor="budget-input" className="block text-sm font-medium text-muted-foreground mb-1">
          追加予算
        </label>
        <div className="flex items-center">
          <div className="relative flex items-center">
            ...
            <input
              id="budget-input"
              type="number"
              className="pl-7 pr-3 py-2 border border-border bg-card text-foreground rounded-md w-40 focus:ring-ring focus:border-ring"
              value={amount}
          ...
          <button
            type="button"
            className="ml-2 px-3 py-2 bg-muted text-foreground hover:bg-accent rounded-md"
          ...
          <button
            type="button"
            className="ml-4 bg-primary-500 text-white px-4 py-2 rounded-md hover:bg-primary-600"
            onClick={handleApply}
          >
            適用
          </button>
      ...
      {/* 通貨に応じたプリセットボタン（更新） */}
      <div className="mb-4">
        <label id="preset-label" className="block text-sm font-medium text-muted-foreground mb-1">
          予算プリセット
        </label>
        <div className="flex flex-wrap gap-2" role="group" aria-labelledby="preset-label">
          {presetAmounts.map((presetAmount) => (
            <button
              key={presetAmount}
              type="button"
              className="bg-primary-500/10 text-primary-500 px-3 py-1 rounded-md hover:bg-primary-500/20 border border-primary-500/30"
      ...
      <div className="bg-muted p-3 rounded-md border border-border">
        <div className="text-sm text-muted-foreground mb-1">現在の設定:</div>
