...
const AllocationEditor = () => {
  ...
  useEffect(() => {
    ...
    targetPortfolio.forEach((item: any) => {
      initialAllocations[item.id] = item.targetPercentage;
    });
    setAllocations(initialAllocations);

    // 合計値の計算
    const total = Object.values(initialAllocations).reduce((sum: number, value: number) => sum + value, 0);
  ...
  const handleAllocationChange = (id: string, value: string) => {
    ...
    // 合計値の再計算
    const total = Object.values(newAllocations).reduce((sum: number, value: number) => sum + value, 0) as number;
    setTotalAllocation(total);
  ...
  const handleAdjustAllocations = () => {
    ...
    const total = Object.values(allocations).reduce((sum: number, value: number) => sum + value, 0);
    if (total === 0) {
      // すべてが0の場合は均等に分配
      ...
      Object.entries(allocations).forEach(([id, value]: [string, any]) => {
      ...
      // 端数調整（最後の項目で調整）
      if (targetPortfolio.length > 0) {
        const adjustedTotal = Object.values(newAllocations).reduce((sum: number, value: number) => sum + value, 0);
