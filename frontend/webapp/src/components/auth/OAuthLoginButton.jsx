...
const OAuthLoginButton = () => {
  ...
  // 既にログイン済みの場合は何も表示しない（UserProfileコンポーネントがHeaderで表示される）
  if (isAuthenticated) {
    return null;
  }
  ...
  if (!googleClientId || googleClientId === 'your_google_client_id' || googleClientId === 'dummy-client-id') {
    return (
      <div className="w-8 h-8 bg-dark-300 rounded-lg animate-pulse flex items-center justify-center">
  ...
  return (
    <div className="flex items-center space-x-2">
      <button
        ...
        className="inline-flex items-center px-3 py-2 border border-dark-400 text-sm font-medium rounded-lg text-gray-300 bg-dark-300 hover:bg-dark-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-primary-400 focus:ring-offset-2 focus:ring-offset-dark-200 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        title="Google Drive連携でデータを安全に保存"
      >
        ...
        {loading || isProcessing ? '処理中...' : 'ログイン'}
      </button>
      
      {(loginError || error) && (
        <div className="fixed top-4 right-4 bg-red-500/90 backdrop-blur border border-red-400 text-white px-4 py-3 rounded-lg shadow-lg z-50 max-w-sm">
          <p className="font-medium text-sm">ログインエラー</p>
