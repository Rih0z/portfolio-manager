# ポートフォリオマネージャー インターフェース仕様書（リファクタリング版）

**最終更新日時:** 2025-05-12 16:00
...
### 5.1 データソース構成
- **市場データAPI**: 単一のAPIサーバー（`REACT_APP_MARKET_DATA_API_URL`）
  - 米国株、日本株、投資信託、ETFの市場データを一元的に提供
  - 銘柄タイプに応じて適切なエンドポイントを構築
...
### 5.3 APIエンドポイント
- **基本URL**: `${REACT_APP_MARKET_DATA_API_URL}/${API_STAGE}/api/market-data`
- **主要パラメータ**:
  - `type`: データタイプ ('us-stock', 'jp-stock', 'mutual-fund', 'exchange-rate', 'fund-info', 'dividend-info')
  - `symbols`: ティッカーシンボル（複数の場合はカンマ区切り）
  - `refresh`: キャッシュをバイパスして最新データを取得するかどうか ('true'/'false')
...
- **最終バックアップ**: 環境変数 `REACT_APP_DEFAULT_EXCHANGE_RATE` のデフォルト値（150.0）
- **取得タイミング**:
  - アプリケーション起動時
  - 通貨切替時
  - データ更新ボタンクリック時
