...
### 3. データのインポート/エクスポート
- **CSV取込ウィザード**: SBI証券、楽天証券の保有証券CSVを自動パース（Shift-JIS自動検出対応）
- 汎用CSV、JSON、YAML形式もサポート
- Google Driveとの連携・自動バックアップ

### 4. ポートフォリオスコア
- **8指標の総合評価**: 分散度、目標適合度、コスト効率、リバランス健全度、通貨分散、配当効率、アセット分散、データ鮮度
- 100点満点のスコアリング
- Free プラン: 基本3指標 / Standard プラン: 詳細8指標
