...
**PortfolioWise**の最も革新的な機能は、あなたの投資ポートフォリオデータを自動的にAI分析用のプロンプトに変換する機能です。
...
- 生成されたプロンプトは**どのAIでも使用可能**（Claude、ChatGPT、Gemini など好きなAIで利用できます）

つまり、あなたは**ボタン1つ**で、あなただけの資産状況に基づいた専門的な投資分析と提案を受け取ることができます。世界の経済情勢やトレンドを反映した最新の分析に基づいて、最適な投資判断が可能になります。
...
PortfolioWiseは、投資家のためのスマートなポートフォリオ管理ツールです。複数のデータソースから株価情報を取得し、資産配分の視覚化、リバランス計算、手数料・配当の分析などの包括的な機能を提供します。さらに、**AIプロンプト生成機能**により、あなたの投資ポートフォリオデータを基にした最適なプロンプトを作成し、お好みのAIアシスタントで自由に投資分析ができます。通貨に合わせた表示（円/ドル）やデータの自動取得により、投資判断をより簡単かつ正確にサポートします。
...
- **自由なAI選択**: 生成されたプロンプトをClaudeやChatGPT、Geminiなど、好きなAIで利用可能
...
- 日本株: スクレイピング（Yahoo Finance Japan、Minkabu、Kabutan）
...
新たに実装されたAIプロンプト生成機能は、あなたのポートフォリオデータを活用して、最適な分析プロンプトを生成します。**PortfolioWise自体はAI分析を行わず、あなたが好きなAIを使って自由に分析できるようサポートします。** この設計には重要な理由があります：アプリがユーザーの資産データを一切保存・管理せず、センシティブな金融情報をアプリ外に送信しないことで、プライバシーとセキュリティを最大限に確保しています。
...
#### 2. 最新情報でリアルタイム分析
- 生成したプロンプトをAIに渡すと、AIが最新の市場動向や世界情勢を調査
...
#### プロンプトコピー画面
ワンクリックであなたのポートフォリオデータを組み込んだプロンプトをコピー。お好みのAIアシスタントに貼り付けるだけで、あなた専用の投資分析を受け取れます。
...
3. お好みのAIアシスタント（Claude、ChatGPT、Geminiなど）を開く
4. コピーしたプロンプトを貼り付けて送信
5. AIが最新情報を元にあなたのポートフォリオを分析
...
1. **データ取得の制約**
   - スクレイピングはサイト構造変更に脆弱
   - 無料APIには使用制限があり、多数の銘柄を一度に更新する際にエラーが発生する可能性あり
