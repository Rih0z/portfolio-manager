...
このプロジェクトはモノレポ構造で、フロントエンドとバックエンドが同じリポジトリ内で管理されています：

```
portfolio-manager/
├── frontend/        # フロントエンドアプリケーション
│   └── webapp/      # React ベースのWebアプリケーション
├── backend/         # バックエンドAPI（AWS Lambda）
...
### バックエンド開発
バックエンド機能はAWS上で稼働しており、継続的に改善を行っています：
- **技術スタック**: AWS Lambda, API Gateway, DynamoDB
- **稼働状況**: 本番環境で安定稼働中
- **API エンドポイント**: https://gglwlh6sc7.execute-api.us-west-2.amazonaws.com/prod
