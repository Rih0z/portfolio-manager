...
describe('run-tests.sh report options', () => {
  ...
  test('--visual option opens report', () => {
    withTempSetup(tmp => {
      ...
      const res = spawnSync('bash', [path.join(tmp, 'script/run-tests.sh'), '--visual', 'unit'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('--junit adds reporter', () => {
    withTempSetup(tmp => {
      ...
      fs.writeFileSync(path.join(bin, 'cross-env'), '#!/bin/sh\nshift\n"$@"\n', { mode: 0o755 });
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'script/run-tests.sh'), '--junit', 'unit'], { cwd: tmp, env, encoding: 'utf8' });
  ...
  test('--validate-coverage succeeds with files', () => {
    withTempSetup(tmp => {
      ...
      fs.writeFileSync(path.join(bin, 'cross-env'), '#!/bin/sh\nshift\n"$@"\n', { mode: 0o755 });
      const env = { ...process.env, PATH: `${bin}:${process.env.PATH}` };
      const res = spawnSync('bash', [path.join(tmp, 'script/run-tests.sh'), '--validate-coverage', '-d', 'unit'], { cwd: tmp, env, encoding: 'utf8' });
      expect(res.status).toBe(0);
